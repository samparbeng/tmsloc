				...
				<div class="row">
					<div class="col-md-12">
						...
						<div class="panel panel-default">
							<div class="panel-heading">View Pending Reservations</div>
							<div class="panel-body table-responsive">
							<?php if($error){?><div class="errorWrap"><strong>ERROR</strong>:<?php echo htmlentities($error); ?> </div><?php } 
				else if($msg){?><div class="succWrap"><strong>SUCCESS</strong>:<?php echo htmlentities($msg); ?> </div><?php }?>
								<table data-toggle="table" data-url="your-json-url" id="tblpendingres" name="tblpendingres" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
									<thead>
										<tr>
											<th>#</th>
											<th>START</th>
									...
									<?php $sql = "SELECT ashase, awie, compname, duration, destination, assignedvehicle, assigneddriver, vtype, confirm, mypr from reservations WHERE confirm = 'Pending' order by ashase";
									$query = $dbh -> prepare($sql);
									$query->execute();
											...
											$thisresid =  $result->mypr;
												if(!empty($result->confirm))
												{
													...
													switch ($actasc)
													{
														...
														case "In Progress":
														echo '<button name="btnrid" class="btn btn-sm btn-success" type="submit" id="btnrid">';
														echo $thisresid;
														echo '</button>';
															break;
