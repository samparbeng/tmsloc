...
<head>
	...
	<!--link rel="stylesheet" href="css/bootstrapper.css"-->
  <style>
		.errorWrap {
    padding: 10px;
    margin: 0 0 20px 0;
				...
				<div class="row">
					<div class="col-sm-12">
						...
						<div class="panel panel-default">
							<div class="panel-heading">View All Reservations</div>
							<div class="panel-body">
							<?php if($error){?><div class="errorWrap"><strong>ERROR</strong>:<?php echo htmlentities($error); ?> </div><?php } 
				else if($msg){?><div class="succWrap"><strong>SUCCESS</strong>:<?php echo htmlentities($msg); ?> </div><?php }?>
								<div class="table-responsive">
								<table id="zctb" class="table table-responsive table-striped table-bordered table-hover" cellspacing="0">
									<thead>
										<tr>
											<th>#</th>
											<th>START</th>
									...
									<?php $sql = "SELECT ashase, awie, compname, duration, destination, assignedvehicle, assigneddriver, confirm, mypr from reservations order by ashase";
									$query = $dbh -> prepare($sql);
									$query->execute();
										...
										<tr>
											...
											$thisresid =  $result->mypr;
												if(!empty($result->confirm))
												{
													...
													{
														case "Pending":
														echo '<button name="btnrid" class="btn btn-block btn-warning" type="submit" id="btnrid">';
														echo $thisresid;
														echo '</button>';
															break;

														case "In Progress":
														echo '<button name="btnrid" class="btn btn-block btn-success" type="submit" id="btnrid">';
														echo $thisresid;
														echo '</button>';
															break;

														case "Closed":
														echo '<button name="btnrid" class="btn btn-block btn-primary" type="submit" id="btnrid">';
														echo $thisresid;
														echo '</button>';
															break;
														...
														echo '<button name="btnrid" class="btn btn-block btn-danger" type="submit" id="btnrid">';
														echo $thisresid;
														echo '</button>';
							...
							<div>
								<button id="btclose" type="button" class="btn btn-red btn-md btn-danger" onClick="parent.location='dashboard.php'">GO BACK</button>
							</div>	
						</div>
					</div>
				</div>
